...
$(document).ready(function() {
	...
	$("#submitTestConnexion").click(function(){
		var log = $("#NETEVEN_LOGIN").val();
		var pass = $("#NETEVEN_PASSWORD").val();
		...
		$.ajax({
			type: "POST",
			url: moduleDir + "ajax/check-connexion.php",
			data: {token:neteven_token, log:log, pass : pass},
			error:function(msg){
				$("#conf-connexion-error").slideDown();
				$(this).removeAttr('disable');
			},
			success:function(reponse){
				if (reponse == '1') {
					$("#conf-connexion-success").slideDown();
					$(this).removeAttr('disable');
				} else {
					$("#conf-connexion-error").slideDown();
					$(this).removeAttr('disable');
	...
	$('.prices_select').on('change', function(){
		...
		var isCountryLinked = $(this).find("option:selected").attr('data-countryLinked');

		if(parseInt(isCountryLinked) > 0) {
